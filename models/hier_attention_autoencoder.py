...
class HierarchicalAutoencoder(object):
    ...
    def build_model(self):
        ...
        with tf.variable_scope("encode_RNN_word"):
            for sent_encode in sent_encodes:
                if t > 0: tf.get_variable_scope().reuse_variables()
                (cell_output, sent_state) = self.encode_sent_cell(sent_encode,
                                                                  sent_state)
        ...
        # DECODE
        #self.decode_sent_cell = BasicLSTMCell(2*self.size)
        ...
        with tf.variable_scope("decode_RNN_sentence"):
            for t1 in range(self.doc_steps):
                ...
                for sent_encode in sent_encodes:
                    ...
                    b1 = tf.get_variable("b1", [self.size, 1])
                    vi = tf.matmul(u, tf.tanh(tf.matmul(w1, sent_decodes[t1]) + tf.matmul(w2, sent_encode) + b1))
                    vi = tf.exp(vi)
                    vs.append(vi)
                ...
                if t1 > 0: tf.get_variable_scope().reuse_variables()
                sent_decode = tf.concat(0, [sent_decode, mt])
